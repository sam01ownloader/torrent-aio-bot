  ...
  bot.onText(downloadRegex, (msg, match) => {
    ...
    const reply = async torr => {  
      ...
      if (!torr.done) {
        ...
        let sam = `https://team_killshot.sam-makkrkill.workers.dev/0:/upload_bot/${torr.name}/`;
          
        var ur1 = encodeUrl(sam);  
    ...
    const onDriveUpload = (torr, url) => bot.sendMessage(from, `${torr.name} uploaded to drive\n${url}\nencodeUrl(`https://team_killshot.sam-makkrkill.workers.dev/0:/upload_bot/${torr.name}/`)`;
